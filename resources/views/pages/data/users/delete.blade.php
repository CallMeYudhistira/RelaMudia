        ...
        <div class="p-6">
            ...
            <div class="mt-4 text-center">
                ...
                <p class="mt-2 text-sm text-gray-500">
                    Apakah Anda yakin ingin menghapus <strong>{{ $user->name }}</strong>? Tindakan ini tidak dapat
                    dibatalkan.
                </p>
            </div>

            {{-- Info detail user yang akan dihapus --}}
            <div class="mt-4 rounded-lg bg-gray-50 p-3">
                <label class="text-xs font-semibold uppercase text-gray-400">Nama User</label>
                <p class="text-sm font-medium text-gray-700 mt-1">{{ $user->name }}</p>
            </div>
        </div>

        <div class="flex flex-col gap-2 bg-gray-50 px-6 py-4 sm:flex-row-reverse">
            <form action="{{ route('users.destroy', $user->id) }}" method="POST" class="w-full sm:w-auto">
                @csrf
                @method('DELETE')
                <button type="submit"
