    ...
    <div @click.away="openDelete = false" x-show="openDelete" x-transition:enter="transition ease-out duration-300"
        ...
        <div class="p-6">
            ...
            <div class="mt-4 text-center">
                <h3 class="text-lg font-semibold text-gray-900">Hapus Item?</h3>
                <p class="mt-2 text-sm text-gray-500">
                    Apakah Anda yakin ingin menghapus <strong>{{ $item->name }}</strong>? Tindakan ini tidak dapat
                    dibatalkan.
                </p>
            </div>

            <div class="mt-4 rounded-lg bg-gray-50 p-3">
                <label class="text-xs font-semibold uppercase text-gray-400">Nama Item</label>
                <p class="text-sm font-medium text-gray-700 mt-1">{{ $item->name }}</p>
            </div>
            <div class="mt-4 rounded-lg bg-gray-50 p-3">
        ...
        <div class="flex flex-col gap-2 bg-gray-50 px-6 py-4 sm:flex-row-reverse">
            <form action="{{ route('items.destroy', $item->id) }}" method="POST" class="w-full sm:w-auto">
                @csrf
                @method('DELETE')
                <button type="submit"
